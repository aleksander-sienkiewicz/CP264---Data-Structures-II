...
// The report_message() routine is called to output an appropriate "MSG_INFO" message if there are no lines of text to print.
{
   ...
   for(int i = 0; i < MAX_MSG_NUM; i++){
   for (int j=0; j<MAX_MSG_LNG; j++){
   msg[i][j]=NULL;//does not change, establish same buffer
   }
   }

   while(nlines!=0){
      for(int i=nlines; i >=0; i--){
         printf("%s\n, p_text[i]");//same same just reverse orfer of for loop to itterate backwards instead
         break;
      }
   }
   if (nlines == 0){
      printf(msg[0],"%s\n", "Error: Line does not exist");
      report_message(msg, MSG_INFO);//does not change as well
   }
   return;
...
static   char indent[]       = "       : ";
   ...
   switch (msg_kind) {                                  // Switch on "msg_kind" to determine which message prefix to output!
      ...
      case MSG_ERROR:
         ...
         break;
      case MSG_END:
         printf("==> END: Program Terminated.\n       : Have a great day.");//new err message. 
         break;
