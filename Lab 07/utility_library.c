...
// NOTE: push() and pop() are not used in this utility routine in order to avoid an infinite calling loop.
{
   ...
   switch (msg_kind)
   { // Switch on "msg_kind" to determine which message prefix to output!
   case MSG_INFO:
      printf(info_prefix);
...
// NOTE: push() and pop() are not used in this utility routine in order to avoid an infinite calling loop.
{
   ...
   if (p_tos < p_mxs)
   { // Check if we have room on the Stack to push a proc_name.
...
//  NOTE: push() and pop() are not used in this utility routine in order to avoid an infinite calling loop.
{
   ...
   if (p_tos > p_bos)
   {                          // Have at lease ONE value in the stack, so let's pop it by simply
