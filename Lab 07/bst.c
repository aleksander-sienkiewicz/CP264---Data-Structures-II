/*
 ...
 -------------------------------------------------------
 St Name: Aleksander Sienkiewicz
 ID: 210222490
 Email: alekssnkwcz@gmail.com
 Date: march 8, 2023
 -------------------------------------------------------
 */
...
//=================================
{
	...
	int nodeLocated = 0;// 0 if not found, 1 if true

	while(node && !nodeLocated){//while node exists, node not found
		//print node
		printf("value: %s\n", tree->to_string(string, DATA_STRING_SIZE, node -> value));
		int comphair = tree -> compare(node -> value, key);//compare node value to value

		if(comphair == 0){ //two nodes are the same
			nodeLocated = TRUE;
			value = tree -> copy(node->value);
		} else if(comphair < 0){//if value is lower
			node = node -> left;//go left, lower value
		
		} else if(comphair > 0){//if value is higher
			node = node -> right;//go right, higher value
		}
	}
	return (value);
}
