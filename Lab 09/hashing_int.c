...
//           - have not yet found an empty slot.
{
	...
	// <<<< Your Code >>>>
	int i = 0;
	int indexValue = ((key % MAX_HASH_TABLE_SIZE) + i) % MAX_HASH_TABLE_SIZE;

	while (i < MAX_HASH_TABLE_SIZE){
		if (hash_table[indexValue] != NULL){
			printf("%4d      Yes\n", indexValue);
		}else{
			break;
			}
		...
		i++;
		indexValue = ((key % MAX_HASH_TABLE_SIZE) + i) % MAX_HASH_TABLE_SIZE;}
