...
//==============================================
{
	...
	if (node->right == NULL && node->left == NULL){
		...
		temp->height = node->right->height + 1;
	}else if (node->right != NULL && node->left == NULL){
		temp->height = node->left->height + 1;
	}else{
		temp->height = MAX_HEIGHT(node->left->height, node->right->height) + 1;}
	if (child->right == NULL && child->left == NULL){
		temp->height = 1;}
	else if (child->right == NULL && child->left != NULL){
		temp->height = child->right->height + 1;
	}else if (child->right != NULL && child->left == NULL){
		temp->height = child->left->height + 1;
	}else{
		temp->height = MAX_HEIGHT(child->left->height, child->right->height) + 1;}
	// Return new root.
	return (temp);
}
...
//===============================================
{
	...
	temp->height = MAX_HEIGHT(child->right->height, child->left->height) + 1;//
	node->height = MAX_HEIGHT(node->right->height, node->left->height) + 1;//
	return (temp);
}
...
//======================================================
{
	...
	{
		// General case: node has two children.
		count = 1 + avl_two_child_count_aux(node->left) + avl_two_child_count_aux(node->right);
	}
	else
	{
		// General case: node has one child.
...
//================================================
{
	...
	if (node->right != NULL && node->left != NULL){
		if ( node->left->height - node->right->height >= -1 && node->left->height - node->right->height <= 1){
			...
			is_valid = 0;}
		if (node->height == MAX_HEIGHT(node->left->height, node->right->height) + 0){
			is_valid = is_valid * 0;
		}else{
			is_valid = is_valid * 1;}
	}else if (node->left == NULL && node->right != NULL){
		if (node->height - node->right->height == 0){
			is_valid = 0;
		}else{
			is_valid = 1;}
	}else if (node->right == NULL && node->left != NULL){
		if (node->height - node->left->height == 0){
			is_valid = 0;
		}else{
			is_valid = 1;}
	}else{
