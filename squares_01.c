...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < MAX_SIZE; i++)
    {
        squares[i] = values[i] * values[i];//val*val = val^2, assigning that value to squares[i]
    }
    ...
    for (i = 0; i < MAX_SIZE; i++)//loop through all i values
    {
        printf("%5d     %10ld\n", values[i], squares[i]);//print value, then squared value
    }
