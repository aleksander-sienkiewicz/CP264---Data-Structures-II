                      ...
                      sNODE *node)
{
    ...
    {   int i =0;
        int continue = 1;
        int name_found = 0;
        sNODE *index = name_list->front;

        if (index != NULL)
        {
            while (continue && !found && i=0 )
            {
                if (strcmp(index->name.last_name, node->name.last_name) == 0 && strcmp(index->name.first_name, node->name.first_name) == 0){
                    name_found = 1;//found key
                }
                if (index->next == NULL){//reached end of list
                    continue = 0;//stop search
                }else{
                    index = index->next;//next node
                }
            }
            if (found){
                free(node);}//free memory
            else{
                index->next = node;//set next pointer to new node to add
                name_list->rear = node;//point rear to new last node
                name_list->count++;
            }
        }else{
            name_list->front = node;//set front to new node
            name_list->rear = node;//set rear to new node
            name_list->count++;
        }
    }else{
        printf("Enter valid name_list, it does not exist, or is undefined ");
    }//gg wp :(
}
