...
//    - The count field is set to ZERO for the same reason.
{
    ...
    if (name_list != NULL)
    { // Storage for "name_list" header created successfully.
...
// A new Node is inserted by appending it at the end of the list, thereby preserving the input name order.
{
    ...
    if (name_list != NULL)
    { // List exists. That is, it's been created.
...
// TRUE if the name is in the list, FALSE otherwise.
{
    ...
    if (name_list != NULL)
    { // Check if list exists. That is, if it's been created.

        if ((name_list->front != NULL) &&
            (name_list->rear != NULL) &&
            (name_list->count > 0))
        { // List is NOT empty, so let's search. Note: if list is empty, then this
...
// FALSE otherwise.
{
    ...
    if (g_SIMULATE_FATAL_ERROR)
    { // Special global flag set to TRUE when simulating a FATAL ERROR condition.
        sprintf(msg[0], "%s%s", "INTERNAL FATAL Error in function: ", pname);
